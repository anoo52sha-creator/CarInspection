...
// // --- API ENDPOINTS ---
// app.post("/api/create-report", upload.array('images', 12), async (req, res) => {
//   try {
//     const reportData = JSON.parse(req.body.reportData);
//     const files = req.files;
//     const referenceNo = `AV${Date.now()}`;
...
//         const result = await cloudinary.uploader.upload(dataURI, { folder: "autovera_reports" });
//         imageUrls.push(result.secure_url);
//       }
//     }
//     reportData.imageUrls = imageUrls;
//     reportData.reportInfo.referenceNo = referenceNo;

//     // 2. Save the complete report object to the database
//     const dbResult = await pool.query(
//       `INSERT INTO vehicle_reports (vin, registration, report_data) VALUES ($1, $2, $3) RETURNING id`,
//       [reportData.carDetails.vin, reportData.carDetails.registration, reportData]
//     );
//     const reportId = dbResult.rows[0].id;
    
//     console.log(`✅ Report ${reportId} created for VIN ${reportData.carDetails.vin}`);
...
//     res.status(500).json({ error: "Failed to create report on server" });
//   }
// });
...
// pool.connect()
//   .then(() => {
//     app.listen(PORT, () => console.log(`✅ Server & DB connected, running on port ${PORT}`));
//   })
